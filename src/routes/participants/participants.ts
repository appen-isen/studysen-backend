...
import { getAllParticipants, getParticipantsByEventId, getParticipantsByEmail, getParticipantsByEventIdAndEmail } from "./getParticipants";
import {postParticipants} from "./postParticipants";
import {putParticipantsEmail, putParticipantsEventId} from "./putParticipants";
...
router.get("/event/:event_id/email/:email", async (req, res) => {
    try {
        const event_id = parseInt(req.params.event_id);
        const email = req.params.email;
        const participants = await getParticipantsByEventIdAndEmail(event_id, email);
        res.status(200).json({ message: participants});
    } catch (error) {
        res.status(500).json({ message: 'Internal server error', error: error });
    }
