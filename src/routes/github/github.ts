...
router.post("/", async (req, res) => {
    try {
        ...
        if (!githubIssueToken) {
            res.status(500).json({ message: 'Internal server error: Github token is not defined' });
            return;
        }
        const response = await fetch("https://api.github.com/repos/appen-isen/isen-orbit/issues", {
            method: "POST",
            headers: {
