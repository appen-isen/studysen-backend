		...
		if (image) {
			...
			try {
				...
				try {
					...
					await fetch(
						`https://api.github.com/repos/${repoOwner}/${repoName}/contents/images/.gitkeep`,
						{
							method: "PUT",
							headers: {
								Authorization: `Bearer ${githubIssueToken}`,
				...
				if (!isValidBase64(cleanedBase64)) {
					throw new Error(
						"Le contenu de l'image n'est pas un base64 valide"
					);
				}
				...
				if (!imageUploadResponse.ok) {
					...
					throw new Error(
						`Échec du téléchargement de l'image: ${errorData}`
					);
				}

				const imageData: ImageUploadResponse =
